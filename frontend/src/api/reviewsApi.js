...
export const getReviewsByUserId = (userId) =>
  apiFetch(`${API_URL}?userId=${userId}`, { errorMsg: '사용자 리뷰 목록을 불러오지 못했습니다.' });

export const addReview = (review) =>
  apiFetch(API_URL, { method: 'POST', body: review, errorMsg: '리뷰 등록에 실패했습니다.' });

export const updateReview = (reviewId, review, requesterId) =>
  apiFetch(`${API_URL}/${reviewId}${requesterQuery(requesterId)}`, {
    method: 'PATCH',
    body: review,
    errorMsg: '리뷰 수정에 실패했습니다.',
  });

export const deleteReview = (reviewId, requesterId) =>
  apiFetch(`${API_URL}/${reviewId}${requesterQuery(requesterId)}`, {
    method: 'DELETE',
    returnJson: false,
    errorMsg: '리뷰 삭제에 실패했습니다.',
  });
