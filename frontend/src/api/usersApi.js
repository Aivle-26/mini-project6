...
export const updateLibraryVisibility = (userId, isPublic) =>
  apiFetch(`${BASE}/${userId}/library-visibility`, {
    method: 'PATCH',
    body: { libraryPublic: isPublic },
    errorMsg: '서재 공개 설정 변경에 실패했습니다.',
  });

export const deleteUser = (userId) =>
  apiFetch(`${BASE}/${userId}`, {
    method: 'DELETE',
    returnJson: false,
    errorMsg: '회원 탈퇴에 실패했습니다.',
  });

export const getUserBooks = (userId) =>
  apiFetch(`/api/books/user/${userId}`, { errorMsg: '공개 서재를 불러오지 못했습니다.' });
